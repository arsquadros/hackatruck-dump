...
class ViewModel: ObservableObject {
    ...
    func fetchNotes() {
        guard let url = URL(string: "http://127.0.0.1:1880/notes/read") else {
            print("Invalid URL")
            return
        }
    ...
    func fetchReminders() {
        guard let url = URL(string: "http://127.0.0.1:1880/reminders/read") else {
            print("Invalid URL")
            return
        }
        let task = URLSession.shared.dataTask(with: url) { [weak self] data, _, error in
    ...
    func uploadNote(note: Notes) {
        guard let url = URL(string: "http://127.0.0.1:1880/notes/create") else {
    ...
    func uploadReminder(reminder: Reminders) {
        guard let url = URL(string: "http://127.0.0.1:1880/reminders/create") else {
